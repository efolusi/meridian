...
**A general-purpose open-source design system by [Efolusi](https://efolusi.com).** 115 accessible React components, 166 design tokens in light, dark and compact, 9 blocks, and 8 full example apps — served browser-native with **no build step**.
...
[![Components](https://img.shields.io/badge/components-115-A0693C.svg)](https://meridian.efolusi.com/site/Components.dc.html)
...
- `components/overlay/` — composable DropdownMenu, Popover, Drawer, Command (+Dialog, +Input, +List, +Empty, +Group, +Item, +Shortcut, +Separator), ContextMenu, HoverCard (+Trigger, +Content), Portal
...
- `components/<group>/` — the 115 components; each ships `.jsx` + `.d.ts` + `.prompt.md` + a group card HTML
