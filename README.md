...
**A general-purpose open-source design system by [Efolusi](https://efolusi.com).** 120 accessible React components, 166 design tokens in light, dark and compact, 9 blocks, and 8 full example apps — served browser-native with **no build step**.
...
[![Components](https://img.shields.io/badge/components-120-A0693C.svg)](https://meridian.efolusi.com/site/Components.dc.html)
...
- `components/feedback/` — Dialog, ConfirmDialog, Sonner (`Toaster` + `toast`), deprecated Toast composition, Tooltip, Alert, Banner, Progress, Spinner, Loader
...
- `components/<group>/` — the 120 components; each ships `.jsx` + `.d.ts` + `.prompt.md` + a group card HTML
