...
- **Transient, after an action**: `toast.add({ title: message, type: 'error' })` through a mounted `Toaster`. A notification that carries an action never auto-dismisses (WCAG 2.2.1); a plain one fades.
...
- Report the result: `toast.add({ title: message, type: 'success' })` on success, an `Alert` if it half-failed.
