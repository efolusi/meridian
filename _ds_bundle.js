...
function Icon({
  ...
  React.useEffect(() => {
    ...
    let live = true;
    fetch(base() + 'assets/icons/' + name + '.svg').then(r => r.ok ? r.text() : Promise.reject(new Error('HTTP ' + r.status))).then(t => {
      _cache[name] = t;
      if (live) setSvg(t);
    }).catch(err => {
      _cache[name] = '';
