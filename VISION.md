...
- **Kept English by design:** component names (proper nouns) and their per-component reference descriptions; the footer (required attribution copy that wraps links); the Preview/Code/Edit demo controls. **Growing:** translating the per-component descriptions is the open depth item — a large content pass, not a foundation gap.
