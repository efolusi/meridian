...
Meridian is a general-purpose, open-source design system by Efolusi: 122 accessible React components, 167 design tokens in light, dark and compact, 12 blocks, and full example apps. It ships two ways from one source. The flagship is zero-build: four tags, one global, no bundler, the way meridian.efolusi.com serves itself. The second is npm, a monorepo of lockstep packages: the `@efolusi/meridian` umbrella (everything: components, tokens, icons, types), plus `@efolusi/meridian-tokens` and `@efolusi/meridian-icons` for teams who want only one layer, real ES modules for Vite, Next and Remix. It is warm paper, espresso ink, hairline structure, with a deliberately abstract three-stroke Meridian mark: the literal shorthand `///`, carrying a sense of forward motion without resolving into a letter or pictogram. It is the studio's own most demanding customer: Efolusi builds its products and its site on it.
