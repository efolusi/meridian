...
- **Landed — bilingual docs site.** `site/i18n.js` is a decoupled en/id layer: a leaf element tagged `data-lang-id` keeps its English inline and carries the Indonesian in the attribute, and the layer swaps them on a language change, re-applying after DCLogic re-renders via a MutationObserver, so it needs no page-logic changes. A `.site-lang` toggle sits in every page header, persists the choice to localStorage, and restores it on load. The home page and the Blocks / Examples / Charts / Themes / Colors hero prose are translated. **Remaining:** Docs and Components render their headings and body from DCLogic state, so their deep prose is still English (a separate content pass); the footer stays English by design (required copy, wraps links).
