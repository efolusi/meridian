...
**Baseline today:** 122 components (source + `.d.ts` + `.prompt.md`), 165 unique token names in light/dark/compact, 118 icons, 12 blocks, 8 example apps, 9 starter journeys, docs site with live demos and search, guidelines (accessibility, forms, governance), MIT license, llms.txt + agent skill.
...
- Custom domain for the docs host (decision point 4) — the repo ships the hosting config (`_headers`, `_redirects`, `404.html`, `robots.txt`, `sitemap.xml`) and OG cards on every docs page; what remains is connecting the repo in the Cloudflare dashboard.
...
- ~~Focus trap, focus-in, restore, labelling for Dialog, Drawer, ConfirmDialog, CommandDialog; keyboard Menu/Popover triggers with arrow navigation; Calendar grid + picker composition; Tabs roving tabindex; Tooltip `aria-describedby`; HoverCard role fix.~~ Done and verified in-browser.
...
- ~~Derive SiteSearch's PAGES/DOCS lists from hand-maintained arrays.~~ Done 2026-07-29: `scripts/build_site_nav.mjs` generates `site/nav.json` with pages derived from the actual `site/*.dc.html` files (a new page is indexed with no array edit — proven with a synthetic page); SiteSearch reads it; wired into `check_all.mjs` with a stale gate. Docs section labels stay a maintained list in the generator (their source is the Docs page runtime). The `_smoke.html` FILES list is a separate item.
