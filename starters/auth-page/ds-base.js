...
(() => {
  ...
  // support.js fetches React before booting a standalone template. Helmet
  // scripts can run during that fetch, so defer the React-dependent bundle.
  const waitForReact = () => window.React ? loadBundle() : setTimeout(waitForReact, 10);
  waitForReact();
})();
