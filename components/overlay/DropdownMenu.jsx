...
export function DropdownMenu({ open: controlled, defaultOpen = false, onOpenChange, modal = true, children }) {
  injectEfCss('ef-css-dropdown-menu', CSS);
  ...
  const value = React.useMemo(() => ({ open, setOpen, triggerRef, contentRef, modal }), [open, setOpen, modal]);
...
export function DropdownMenuPortal({ children, container }) {
...
export const DropdownMenuContent = React.forwardRef(function DropdownMenuContent({ loop = true, align = 'center', side = 'bottom', sideOffset = 4, alignOffset = 0, className, style, onKeyDown, onCloseAutoFocus, children, slot = 'dropdown-menu-content', ...rest }, forwardedRef) {
  ...
  const { style: anchored, side: resolvedSide } = useAnchoredStyle(root?.triggerRef ?? fallbackTriggerRef, root?.contentRef ?? fallbackContentRef, { open: !!root?.open, placement: side, align, offset: sideOffset, crossOffset: alignOffset });
  ...
  useIsoLayoutEffect(() => { if (root?.open) menuItems(localRef.current)[0]?.focus(); }, [root?.open]);
  React.useEffect(() => {
    if (!root?.open) return;
