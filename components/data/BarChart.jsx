...
`;
export function BarChart({ data = [], height = 140, highlightLast = 0, labels, format, formatValue, style, className, ...rest }) {
      ...
      <div className="ef-bars" style={{ height }}>
        {vals.map((v, i) => (
          <div key={i} className={`ef-bars__bar${i >= vals.length - highlightLast ? ' ef-bars__bar--hi' : ''}`}
            ...
            title={(data[i] && data[i].label ? data[i].label + ' · ' : '') + (fmt ? fmt(v) : v.toLocaleString())}></div>
        ))}
      </div>
